...
fnames = glob.glob( '/home/alireza/Documents/cv-lab12/obj?.jpg' )
fnames.sort()
for fname in fnames:
    ...
    M = np.array( [[0, -1], [-1, 0]] )
    t = np.array( [[I1.shape[0] / 2], [I1.shape[1] / 2]] )
    #
    t0=np.array(([0,0,1]))
    #
    t = t - M.dot( t )
    ...
    # I1 = cv2.warpAffine( I1, M1, (I1.shape[1], I1.shape[0]) )
    ...
    pts = np.float32( [[0, 0],
                       ...
                       [I1.shape[1],0]] ).reshape( -1, 1, 2 )  # this needs to be changed

    dst = cv2.perspectiveTransform( pts, H1 ).reshape( 4, 2 )
    ...
    # cv2.circle( J, (dst[0, 0], dst[0, 1]), 5, [0, 0, 255], 2 )
    # cv2.circle( J, (dst[1, 0], dst[1, 1]), 5, [0, 0, 255], 2 )
